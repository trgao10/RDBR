function r=ksr_characteristic(x,y,h,x_regression)
% KSR   Kernel smoothing regression
%
% r=ksr_characteristic(x,y) returns the Gaussian kernel regression in structure r such that
%   r.f(r.x) = y(x) + e
% The bandwidth and number of samples are also stored in r.h and r.n
% respectively.
%
% r=ksr_characteristic(x,y,h) performs the regression using the specified bandwidth, h.
%
% r=ksr_characteristic(x,y,h,n) calculates the regression in n points (default n=100).
%
% Without output, ksr_characteristic(x,y) or ksr_characteristic(x,y,h) will display the regression plot.
%
% Algorithm
...
kerf = @(z) double(z < 1 & z > -1)/2;
% kerf = @(z) double(z < 1 & z > 0);
